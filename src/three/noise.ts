/**
 * Compact 3D simplex noise (Gustavson/Ashima port, public domain).
 *
 * Used instead of Math.random() per frame: random-per-frame reads as TV static,
 * simplex reads as something alive and drifting. Seeded so node layout is
...
export class SimplexNoise {
  ...
  constructor(seed = 1337) {
    ...
    // Deterministic Fisher-Yates driven by a mulberry32 PRNG.
    let s = seed >>> 0;
    const rand = () => {
      s = (s + 0x6d2b79f5) >>> 0;
      let t = s;
